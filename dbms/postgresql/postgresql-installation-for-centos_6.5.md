# PostgreSQL installation \( for CentOS\_6.5 \)

PostgreSQL 공식 홈페이지 다운로드 페이지 참조. [여기](https://www.postgresql.org/download/)

## yum을 이용한 PostgreSQL 설치

공식 홈페이지에서 적용하고자 하는 버전과 플랫폼을 입력하면 설치를 위한 커멘드가 나온다. [Redhat 계열 다운로드 페이지](https://www.postgresql.org/download/linux/redhat/)
...
postgres 계정으로 접속해 보면 기본 홈 경로가 `/var/lib/pgsql`로 잡혀있는 것을 알 수 있다. 이를 사용하고자 하는 유저 홈 경로로 변경해 주자.
...
설정이 끝났으면 postgres 계정으로 다시 접속 후 .bash\_profile을 설정해 준다.
...
다음으로는 `postgre.sqlconf` 파일을 수정해야 한다. 해당 파일에서 listen\_addresses를 주석 해제 후 \*를 적용한다.
